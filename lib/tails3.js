...
BucketFileStream.prototype.checkForNewFiles = function(all){
    ...
    if(!oldest){
        since = bucketDateFormat(self.since);
    }else{
        since = dateFromFilename(oldest);
    }
...
BucketDataStream.prototype.pollFile = function(file){
    ...
    stream.on('data', function(line){
        ...
        if(lines_read > self.lines_read[file]){
            ...
            if((new Date(l.timestamp)).getTime() > self.since.getTime())
                self.lines.push(l);
...
BucketDataStream.prototype.emitAvailableLines = function(){
    ...
    var emit_before = new Date((new Date()).getTime() - 20000);
    var i;
